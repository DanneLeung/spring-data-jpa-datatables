	...
	private static class DataTablesSpecification<T> implements Specification<T> {
		...
		public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
			...
			for (Column column : input.getColumns()) {
				...
				// id equals
				if (column.getData().endsWith(".id")) {
					Expression<Long> idExpression = getExpression(root, column.getData(), Long.class);
					predicate = cb.and(predicate, cb.equal(idExpression, filterValue));
					continue;
				}
			...
			for (Column column : input.getColumns()) {
				...
				for (int i = 0; i < values.length - 1; i++) {
					From<? extends Object, ? extends Object> fetchParent = (fetch == null ? root : fetch);
					boolean met = false;
					if (!fetchParent.getFetches().isEmpty()) {
						for (Fetch<? extends Object, ?> f : fetchParent.getFetches()) {
							if (f.getAttribute().getName().equals(values[i])) {
								met = true;
								break;
							}
						}
						// } else if (!fetchParent.getJoins().isEmpty()) {
						// for (Join<? extends Object, ?> f : fetchParent.getJoins()) {
						// if (f.getAttribute().getName().equals(values[i])) {
						// met = true;
						// break;
						// }
					...
					if (!met)
						fetch = (From<?, ?>) fetchParent.fetch(values[i], JoinType.LEFT);
				}
			}
			return predicate;
		}
